...
class A2CAgent(a2c_common.ContinuousA2CBase):
    def __init__(self, base_name, params):
        ...
        self.has_value_loss = (self.has_central_value and self.use_experimental_cv) or (
            not self.has_phasic_policy_gradients and not self.has_central_value
        )
    ...
    def calc_gradients(self, input_dict):
        ...
        with torch.cuda.amp.autocast(enabled=self.mixed_precision):
            ...
            e_loss_privilige = torch.nn.functional.mse_loss(
                priv_encode, e_state.detach()
            )
            ...
            loss = (
                ...
                + self.estimate_coef
                * (
                    self.estimate_ratio * e_loss_history
                    + (1 - self.estimate_ratio) * e_loss_privilige
                )
            )
    ...
    def train_epoch(self):
        ...
        log_dict = {}
        log_dict["losses/system_identification_loss"] = (
            self.estimate_coef * torch_ext.mean_list(e_losses).item()
        )
    ...
    def train(self):
        ...
        while True:
            ...
            if self.rank == 0:
                ...
                curr_frames = (
                    self.curr_frames * self.rank_size
                    if self.multi_gpu
