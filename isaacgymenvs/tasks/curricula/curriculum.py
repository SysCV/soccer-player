...
class Curriculum:
    ...
    def __init__(self, seed, **key_ranges):
        ...
        for key, v_range in key_ranges.items():
            ...
            cfg[key] = np.linspace(v_range[0] + bin_size / 2, v_range[1] - bin_size / 2, v_range[2]) # centers
        ...
        self.weights_shaped = self.weights.reshape([*self.ls.values()])
...
class RewardThresholdCurriculum(Curriculum):
    ...
    def get_local_bins(self, bin_inds, ranges=0.1):
        """
        The get_local_bins method returns the indices of the adjacent bins given a set of bin indices. It takes bin_inds and ranges as arguments. If ranges is a float, it considers the same range for all dimensions. Otherwise, it expects an array of ranges for each dimension. The method computes the adjacent indices by checking if each dimension of the grid falls within the specified ranges. It returns the boolean mask indicating the adjacent bins.
        """
...
if __name__ == '__main__':
    r = RewardThresholdCurriculum(100, x=(-1, 1, 5), y=(-1, 1, 6), z=(-1, 1, 11))

    assert r._raw_grid.shape == (3, 5, 6, 11), "grid shape is wrong: {}".format(r.grid.shape) # the first dimension is (x, y, z)

    low, high = np.array([-0.2, -0.6, -0.2]), np.array([0.2, 0.6, 0.2])

    r.set_to(low, high, value=1.0)

    samples, bins = r.sample(1000)

    plt.scatter(*samples.T[0:2])
    plt.show()

    plt.imshow(np.sum(r.weights_shaped, axis=2), cmap='gray')
    plt.show()
    ...
    for adjacent in adjacents:
        ...
        print(adjacent_inds)
        # r.update(bin_inds=adjacent_inds, 
