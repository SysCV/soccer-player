...
from isaacgymenvs.tasks.cassie_dribble import CaDribbler
...
class RewardTerms:
    def __init__(self, env: CaDribbler):
        self.env = env
    ...
    def _reward_dribbling_robot_ball_vel(self):
        FR_shoulder_idx = self.env.gym.find_actor_rigid_body_handle(
            self.env.envs[0], self.env.a1_handles[0], "right_hip"
        )
