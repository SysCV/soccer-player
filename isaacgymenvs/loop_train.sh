...
for seed in "${seeds[@]}"; do
    for train_arg in "${train_args[@]}"; do
        python train.py task=Go1 train="$train_arg" wandb_activate=true capture_video=true experiment="$train_arg" seed="$seed" max_iterations=3000 wandb_group="8-21-easier-for-deploy"
    done
done
