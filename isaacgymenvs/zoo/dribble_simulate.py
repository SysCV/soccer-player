...
# Constants
radius = 0.0  # Radius of the larger circular trajectory
angular_speed = 0.0 * math.pi  # Angular speed in radians per second

ball_linear_speed_x = 1.0  # Linear speed of the ball in the x-direction
ball_linear_speed_y = 0.0  # Linear speed of the ball in the y-direction
...
kp_ang = 0.0  # Proportional gain for angular speed control
...
rect_linear_speed = torch.tensor([1.0, 0.0])  # Initial linear speed of the rectangle
...
def update_position(frame):
    if frame < 100:
        ...
        commands = torch.tensor([-1.0, 0.0])
    global rect_pos, rect_linear_speed, rect_angle, rect_angular_speed, prev_error_pos, prev_error_ang, prev_ball_pos, ball_linear_speed_x, ball_linear_speed_y, k1, k2, k3  # Declare variables as global
    ...
    ball_pos = torch.tensor(
        ...
        [ball_linear_speed_x * frame * dt, ball_linear_speed_y * frame * dt]
    )
    ...
    # print("ball pos in frame:{}".format(frame), ball_pos)
    # print("ball speed in frame:{}".format(frame), ball_speed)
    ...
    p_dog_target = ball_pos + delta_v * k1
    v_dog_target = (p_dog_target - rect_pos) * k2 + (
        ball_speed - rect_linear_speed
    ) * k3

    # Update the linear speed of the rectangle
    rect_linear_speed = v_dog_target
